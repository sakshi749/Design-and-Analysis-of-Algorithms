...
void divide_and_conquer(int n, int **A, int **B, int **C) {
    if (n == 1) {
        ...
        divide_and_conquer(new_size, A12, B21, M1);
        matrix_add(new_size, C11, M1, C11);

        divide_and_conquer(new_size, A11, B12, C12);
        divide_and_conquer(new_size, A12, B22, M1);
        matrix_add(new_size, C12, M1, C12);

        divide_and_conquer(new_size, A21, B11, C21);
        divide_and_conquer(new_size, A22, B21, M1);
        matrix_add(new_size, C21, M1, C21);

        divide_and_conquer(new_size, A21, B12, C22);
        divide_and_conquer(new_size, A22, B22, M1);
        matrix_add(new_size, C22, M1, C22);
...
int main() {
    ...
    clock_t start = clock();
    matrix_mult_3loop(n, A, B, C);
    clock_t end = clock();
    printf("3-Loop method time: %.12f seconds\n", get_time_in_seconds(start, end));

    // Measure time for Divide and Conquer method
    start = clock();
    divide_and_conquer(n, A, B, C);
    end = clock();
